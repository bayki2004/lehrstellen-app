...
struct LehrstelleRowView: View {
    let lehrstelle: LehrstelleCard
    ...
    var body: some View {
        HStack(spacing: Theme.Spacing.md) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(lehrstelle.berufTitle)
                    .font(Theme.Typography.callout)
                    .lineLimit(1)
                Text(lehrstelle.companyName)
            ...
            VStack(alignment: .trailing, spacing: 2) {
                Text(lehrstelle.canton)
                    .font(Theme.Typography.badge)
                    .padding(.horizontal, 6)
                    .padding(.vertical, 2)
                    .background(Theme.Colors.primaryFallback.opacity(0.15))
                    .foregroundStyle(Theme.Colors.primaryFallback)
                    .clipShape(Capsule())
                Text(lehrstelle.city)
                    .font(Theme.Typography.caption)
    ...
    private var fieldColor: Color {
        guard let field = lehrstelle.berufCategory else { return Theme.Colors.primaryFallback }
        switch field {
    ...
    private var fieldIcon: String {
        guard let field = lehrstelle.berufCategory else { return "briefcase" }
        switch field {
