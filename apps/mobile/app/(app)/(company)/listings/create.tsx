...
import { CANTONS } from '@lehrstellen/shared';
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chipScroll}>
          {APPRENTICESHIP_FIELDS.slice(0, 12).map((f) => (
            <TouchableOpacity
              key={f.id}
              style={[styles.chip, form.field === f.id && styles.chipSelected]}
              onPress={() => update('field', f.id)}
            >
              <Text style={[styles.chipText, form.field === f.id && styles.chipTextSelected]}>
                {f.label}
              </Text>
            </TouchableOpacity>
          ))}
